...
seurat_object<-subset(seurat_object,Cell_Type!="ASC")
seurat_object<-subset(seurat_object,disease_status=="SS")
SS_cds <- new_cell_data_set(as(as.matrix(seurat_object@assays$RNA@counts), 'sparseMatrix'),
...
SS_cds  <- preprocess_cds(SS_cds , num_dim = 50)
SS_cds  <- reduce_dimension(SS_cds )
SS_cds  <- cluster_cells(SS_cds )
SS_cds  <- learn_graph(SS_cds)
saveRDS(SS_cds,"SS_cds.rds")

cds<-readRDS("SS_cds.rds")
...
AFD_lineage_cds1 <- order_cells(cds, root_pr_nodes=myselect(cds,select.classify = 'Cell_Type',my_select = "ABS"))
...
cds_exprs<-subset(cds_exprs,cds_exprs$Cell_Type!="ASC")
...
pdf("Figure4 pseudotime_SS.pdf",width = 8,height = 6)
